...
async def find_driver_handler(passenger_tg_id: int, active_drivers: List[Driver]) -> Optional[Driver]:
    # получаем пассажира из бд
    passenger = await get_user_data_handler(tg_id=passenger_tg_id)
    if not passenger: # если не нашли, значит нечего обновлять
        return None
    passenger_time = passenger.start_time
    passenger_timedelta = datetime.timedelta(hours=passenger_time.hour, minutes=passenger_time.minute) # переводим в таймдельту, чтобы можно было вычитать и складывать время
    for driver in active_drivers: # перебираем водителей и ищем подходящего
        ...
        if driver_timedelta > passenger_timedelta:
            ...
            if (passenger_timedelta - driver_timedelta).seconds // 60 > 10:
                continue
        active_drivers[driver] -= 1 # если драйвер прошел все проверки, мы отнимаем от количества свободных мест 1
        return driver
    return None
